import 'dart:ffi';
import 'dart:typed_data';

class Encrypter {
  static final Uint8List s_box = Uint8List.fromList([
    
  ]);
  static final Uint8List mul2 = Uint8List.fromList([
    
  ]);
  static final Uint8List mul3 = Uint8List.fromList([
    
  ]);
  static final Uint8List rcon = Uint8List.fromList([
    
  ]);

  String encrypt(String data, String keydata) {
    int originallen = data.length;
    int lenOfPaddedMessage = originallen;
    List message = data.split("");
    List key = keydata.split("");
    List paddedmessage = [];

    if (lenOfPaddedMessage % 16 != 0) {
      lenOfPaddedMessage = ((lenOfPaddedMessage / 16 + 1) * 16) as int;
    }
    for (int i = 0; i < lenOfPaddedMessage; i++) {
      if (i >= originallen)
        paddedmessage[i] = 0;
      else
        paddedmessage[i] = message[i];
    }
    for (int i = 0; i < lenOfPaddedMessage; i += 16) {
      AES_Encrypt(paddedmessage, key);
    }

    for (int i = 0; i < lenOfPaddedMessage; i++) {
      print(paddedmessage[i]);
      print("");
    }
    //delete[] paddedMessage;
    return data;
  }

  void AES_Encrypt(List message, List key) {
    List state = [];
    int number_of_rounds = 9;
    for (int i = 0; i < 16; i++) {
      state.add(message[i]);
    }
    String expandedKey = '';
    KeyExpansion(key, expandedKey);
  }

  void KeyExpansion(List inputkey, var expandedKey) {
    var expandedKeys = expandedKey;
    for (int i = 0; i < 16; i++) {
      expandedKeys[i] = inputkey[i];
    }

    //Variables:
    int bytesGenerated = 16;
    int rconIteration = 1;
    var temp;

    while (bytesGenerated < 176) {
      //read 4 bytes for the core
      for (int i = 0; i < 4; i++)
        temp[i] = expandedKeys[i + bytesGenerated - 4];

      //perform the core once for each 16 byte key
      if (bytesGenerated % 16 == 0) {
        KeyExpansionCore(temp, rconIteration);
        rconIteration++;
      }

      //XOR temp
      for (int a = 0; a < 4; a++) {
        expandedKeys[bytesGenerated] =
            expandedKeys[bytesGenerated - 16] ^ temp[a];
        bytesGenerated++;
      }
    }
    List state;
    Subytes(List);
    Shiftrows(List);
    Addroundkey(List, expandedKey + 160);
    //copy over the encrypted messsage
    for (int i = 0; i < 16; i++) {
      List state = [];
      var message = state;
    }
  }

  void Subytes(state) //
  {
    for (int i = 0; i < 16; i++) {
      state[i] = s_box[state[i]];
    }
  }

  void Shiftrows(state) {
    List tmp = [];
    tmp[0] = state[0];
    tmp[1] = state[5];
    tmp[2] = state[10];
    tmp[3] = state[15];

    tmp[4] = state[4];
    tmp[5] = state[9];
    tmp[6] = state[14];
    tmp[7] = state[3];

    tmp[8] = state[8];
    tmp[9] = state[13];
    tmp[10] = state[2];
    tmp[11] = state[7];

    tmp[12] = state[12];
    tmp[13] = state[1];
    tmp[14] = state[6];
    tmp[15] = state[11];

    for (int i = 0; i < 16; i++) {
      state[i] = tmp[i];
    }
  }

  void Addroundkey(state, List roundkey) {
    for (int i = 0; i < 16; i++) {
      state[i] ^= roundkey[i];
    }
  }

  void KeyExpansionCore(inn, int i) {
    //rotate left
    var t = inn[0];
    inn[0] = inn[1];

    inn[1] = inn[2];
    inn[2] = inn[3];

    inn[3] = t;

    //s-box four bytes
    inn[0] = s_box[inn[0]];
    inn[2] = s_box[inn[2]];
    inn[3] = s_box[inn[3]];

    //Rcon
    inn[0] ^= rcon[i];
  }
}
